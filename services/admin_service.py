...
def finalize_week_logic(db: Session, week_id: int):
    ...
    for user in active_users:
        if user.id not in users_with_order_ids:
            # Estructura corregida para pedidos vacíos
...
def export_week_to_excel(db: Session, week_id: int, office_id: int = None):
    ...
    # Función de búsqueda con manejo de errores de tipo (String vs Int)
    def get_desc(item_id):
        if item_id is None: return None
        try:
            # Intentamos convertir a INT por si viene como String desde el JSON
            val_id = int(item_id)
        except:
            return "ID Inválido"
    ...
    for order in orders:
        # Nos aseguramos de que details sea un diccionario
...
def create_office(db: Session, name: str):
    new_office = Office(name=name.strip())
    db.add(new_office)
    db.commit()
...
def delete_office(db: Session, office_id: int):
    ...
    if office:
        db.delete(office); db.commit(); return True, "Eliminada."
    return False, "Error."